...
def preprocess_function(batch):
    ...
    inputs = tokenizer(batch["event_mention"], padding="max_length", max_length=max_len_source, truncation=True)
    # tokenize the target summary
    outputs = tokenizer(batch["event"], padding="max_length", max_length=max_len_summary, truncation=True)
...
train_dataset_model_input = train_dataset.map(
    ...
    remove_columns=["event_mention", "event"]
)
...
args = Seq2SeqTrainingArguments(
    ...
    logging_steps = 2289//2, # 2289 means every epoch
    save_steps = 2289,
    warmup_steps = 1000,
